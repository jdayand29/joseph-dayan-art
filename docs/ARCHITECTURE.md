...
### Tres catálogos de animación, y por qué
...
- **`ArtworkLightbox`** (Fase I.2) — Motion también, pero con `LazyMotion`+**`domMax`**, no `domAnimation`: es el único lugar del proyecto que necesita `layout`/`layoutId` (transición de elemento compartido — el thumbnail se transforma visualmente en la imagen a pantalla completa), una característica que `domAnimation` no soporta. Reservado desde Fase F (`styles/motion/transitions.ts`, `lightboxTransition: spring.smooth`, comentario explícito anticipando este momento). `domMax` es estrictamente más pesado que `domAnimation` — se aísla a este único componente vía su propio `LazyMotion` (mismo patrón de aislamiento que `Reveal`/`Stagger`), no se usa como reemplazo global; verificado que ninguna ruta que no renderiza `ArtworkLightbox` carga ese bundle. Se construye sobre `@radix-ui/react-dialog` (misma base que `Dialog`/`Drawer`) pasando su propio elemento a `RadixDialog.Content` vía `asChild` (un `m.div`, no un `<div>` plano) — necesario para que Motion pueda rastrear `layout`/`layoutId` sobre el nodo que Radix monta/desmonta; no reutiliza el componente `DialogContent` (su tarjeta blanca centrada no aplica a un visor de imagen a pantalla completa). Bajo `useReducedMotion()`, la transición pasa a `{ duration: 0 }` — el diálogo aparece/desaparece sin el morph.
...
- **`Breadcrumb`** (`components/ui/`, no `components/navigation/`) — pregunta dejada abierta explícitamente en Fase H, resuelta en Fase I.2: se queda en `ui/` porque es genérico (`BreadcrumbItem[]`, sin ningún tipo de dominio) y no depende de `config/navigation.ts` ni de `usePathname` — a diferencia de `NavLink`/`MobileMenu`, que sí están acoplados a la navegación de sitio persistente. Primer consumidor real: `obra/[slug]/page.tsx`, con el trail `Obra / {colección} / {título}` (2 niveles si la obra no pertenece a ninguna colección).
